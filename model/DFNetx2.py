...
class AGU(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        # 使用更大的kernel来捕获红外图像的低频信息
        self.fc1 = CGM(in_features, hidden_features, kernel_size=3, padding=1)
        self.act = act_layer()

        # 添加边缘增强分支
        self.edge_conv = nn.Conv2d(hidden_features, hidden_features, kernel_size=3, padding=1, groups=hidden_features)
        self.edge_weight = nn.Parameter(torch.tensor(0.3))  # 可学习的边缘权重

        self.fc2 = CGM(hidden_features, out_features, kernel_size=3, padding=1)
        self.drop = nn.Dropout(drop)

        # 添加对比度增强
        self.contrast_enhance = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(out_features, out_features // 4, 1),
            nn.ReLU(inplace=True),
            nn.Conv2d(out_features // 4, out_features, 1),
            nn.Sigmoid()
        )
...
class CGM(nn.Conv2d):
    def __init__(self, in_channels=64, out_channels=64, kernel_size=1, padding=0, stride=1, dilation=1, groups=1,
                 bias=True):
        super(CGM, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias)
...
class C_MSA(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, kernel_size=5, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # 初始化权重矩阵
        self.weight = nn.Parameter(torch.randn(num_heads, head_dim, head_dim) * 0.01, requires_grad=True)
        self.to_qkv = CGM(dim, dim * 3, kernel_size=3, padding=1)
        ...
        # 为红外图像添加局部-全局注意力平衡
        self.local_global_balance = nn.Parameter(torch.tensor(0.5))
    ...
    def forward(self, x, k1=None, v1=None, return_x=False):
        ...
        # 处理外部的k和v（用于跨层连接）
        if k1 is not None:
            k = self.local_global_balance * k + (1 - self.local_global_balance) * k1
            v = self.local_global_balance * v + (1 - self.local_global_balance) * v1
...
class CFGT(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        kernel_size2 = 3
        padding2 = 1
        self.attn = C_MSA(dim, num_heads, kernel_size1, padding1)
        self.attn1 = C_MSA(dim, num_heads, kernel_size2, padding2)

        self.norm2 = LayerNorm(dim)
        self.norm3 = LayerNorm(dim)
        mlp_hidden_dim = int(dim*1)
        self.mlp = AGU(in_features=dim, hidden_features=mlp_hidden_dim)
...
class eca_layer_ir(nn.Module):
    def __init__(self, channel, k_size=3):  # 增大k_size到5,捕获更大范围相关性
        super(eca_layer_ir, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.max_pool = nn.AdaptiveMaxPool2d(1)  # 添加最大池化,增强边缘响应
        self.k_size = k_size
    ...
    def forward(self, x):
        ...
        y_max = self.max_pool(x)  # 红外图像中边缘信息更重要
        y = y_avg + y_max  # 融合平均和最大特征
...
class MaskPredictor_IR(nn.Module):
    def __init__(self, in_channels, wn=lambda x: torch.nn.utils.weight_norm(x)):
        super(MaskPredictor_IR, self).__init__()
        # 边缘检测分支
        self.edge_conv = nn.Sequential(
            wn(nn.Conv2d(in_channels, in_channels // 2, 3, 1, 1)),
            nn.LeakyReLU(0.05),
            wn(nn.Conv2d(in_channels // 2, 1, 1))
        )

        self.spatial_mask = nn.Sequential(
            wn(nn.Conv2d(in_channels, in_channels // 2, 3, 1, 1)),
            nn.LeakyReLU(0.05),
            wn(nn.Conv2d(in_channels // 2, 3, 1))  # 输出3通道，用于gumbel_softmax
        )
    ...
    def forward(self, x):
        ...
        # 将边缘特征注入到中间 mask 通道（如原逻辑）
        spa_mask[:, 1:2, :, :] = spa_mask[:, 1:2, :, :] + edge_feat

        # 使用 Gumbel-Softmax 进行离散化
        spa_mask = F.gumbel_softmax(spa_mask, tau=0.5, hard=True, dim=1)
        return spa_mask
...
class FFEM(nn.Module):
    def __init__(self, n_feats, wn=lambda x: torch.nn.utils.weight_norm(x)):
        ...
        self.CA = eca_layer_ir(n_feats, k_size=5)
        self.MaskPredictor = MaskPredictor_IR(n_feats)

        # 使用组卷积减少参数,红外图像信息冗余较少
        self.k = nn.Sequential(
            wn(nn.Conv2d(n_feats, n_feats, 3, 1, 1, groups=n_feats // 4)),  # 深度可分离
            nn.LeakyReLU(0.05),
            wn(nn.Conv2d(n_feats, n_feats, 1, 1, 0))  # 点卷积
        )

        self.k1 = nn.Sequential(
            wn(nn.Conv2d(n_feats, n_feats, 3, 1, 1, groups=n_feats // 4)),
            nn.LeakyReLU(0.05),
            wn(nn.Conv2d(n_feats, n_feats, 1, 1, 0))
        )

        # 红外图像残差连接权重可以更大
        self.res_scale = Scale(1.0)  # 增强残差保留低频信息
...
class LIAM(nn.Module):
    def __init__(self, n_feats, wn=lambda x: torch.nn.utils.weight_norm(x)):
        super(LIAM, self).__init__()
        pooling_r = 2

        # 降低上采样后的噪声放大
        self.k1 = nn.Sequential(
            nn.ConvTranspose2d(n_feats, n_feats * 4 // 3, pooling_r, pooling_r, 0, groups=1, bias=True),
            nn.LeakyReLU(0.05),
            nn.Conv2d(n_feats * 4 // 3, n_feats, 1, 2, 0),
            nn.BatchNorm2d(n_feats)  # 添加BN层稳定训练,抑制噪声
        ) # 这一部分为RFSU

        self.sig = nn.Sigmoid()

        self.k3 = FFEM(n_feats)
        self.k4 = FFEM(n_feats)
        self.k5 = FFEM(n_feats)

        self.res_scale = Scale(1.0)  # 加强残差连接
        self.x_scale = Scale(1.0)
...
class Deepfeature(nn.Module):
    def __init__(self, inplanes, outplanes, stride=1, res_conv=False, act_layer=nn.ReLU, groups=1, norm_layer=nn.BatchNorm2d, drop_block=None, drop_path=None):
        ...
        self.rb_search1 = LIAM(med_planes)
        self.rb_search2 = LIAM(med_planes)
        self.rb_search3 = LIAM(med_planes)
        self.rb_search4 = LIAM(med_planes)
        
        self.trans_block = CFGT(
            dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=False, qk_scale=None,
            drop=0., attn_drop=0., drop_path=0.)#每个里面包含两个GTIAM
            
        self.trans_block1 = CFGT(
            dim=embed_dim, num_heads=num_heads*3//2, mlp_ratio=mlp_ratio, qkv_bias=False, qk_scale=None,
            drop=0., attn_drop=0., drop_path=0.)
            
        self.trans_block2 = CFGT(
            dim=embed_dim, num_heads=num_heads*2, mlp_ratio=mlp_ratio, qkv_bias=False, qk_scale=None,
            drop=0., attn_drop=0., drop_path=0.)
            
        self.trans_block3 = CFGT(
            dim=embed_dim, num_heads=num_heads*3//2, mlp_ratio=mlp_ratio, qkv_bias=False, qk_scale=None,
            drop=0., attn_drop=0., drop_path=0.)
            
        self.trans_block4 = CFGT(
            dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=False, qk_scale=None,
            drop=0., attn_drop=0., drop_path=0.)
            
        self.trans_block5 = CFGT(
            dim=embed_dim, num_heads=num_heads*3//2, mlp_ratio=mlp_ratio, qkv_bias=False, qk_scale=None,
            drop=0., attn_drop=0., drop_path=0.)
            
        self.trans_block6 = CFGT(
            dim=embed_dim, num_heads=num_heads*2, mlp_ratio=mlp_ratio, qkv_bias=False, qk_scale=None,
            drop=0., attn_drop=0., drop_path=0.)
            
        self.trans_block7 = CFGT(
            dim=embed_dim, num_heads=num_heads*3//2, mlp_ratio=mlp_ratio, qkv_bias=False, qk_scale=None,
            drop=0., attn_drop=0., drop_path=0.)
    ...
    def forward(self, x):
        ...
        x = self.squeeze_block(self.trans_block(self.expand_block(self.rb_search1(x)))) + x # 1 CT block
        
        x = self.squeeze_block(self.trans_block1(self.expand_block(self.rb_search1(x)))) + x # 2 CT block
        
        x = self.squeeze_block1(self.trans_block2(self.expand_block1(self.rb_search2(x)))) + x # 3 CT block
        
        x = self.squeeze_block1(self.trans_block3(self.expand_block1(self.rb_search2(x)))) + x # 4 CT block
        
        x = self.squeeze_block2(self.trans_block4(self.expand_block2(self.rb_search3(x)))) + x # 5 CT block
        
        x = self.squeeze_block2(self.trans_block5(self.expand_block2(self.rb_search3(x)))) + x # 6 CT block
        
        x = self.squeeze_block3(self.trans_block6(self.expand_block3(self.rb_search4(x)))) + x # 7 CT block
        
        x = self.squeeze_block3(self.trans_block7(self.expand_block3(self.rb_search4(x)))) + x # 8 CT block
...
class ConvTransBlock(nn.Module):
    ...
    def __init__(self, inplanes, outplanes, res_conv, stride, dw_stride, embed_dim, num_heads, mlp_ratio,
        ...
        expansion = 1
        self.cnn_block = Deepfeature(inplanes=inplanes, outplanes=outplanes, res_conv=res_conv, stride=1, groups=groups)
